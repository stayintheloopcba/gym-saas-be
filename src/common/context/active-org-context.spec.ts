...
describe('AuthContextMiddleware', () => {
  ...
  it('exposes the active org when the user has an active membership', async () => {
    ...
    const context = await run({ access_token: 'tok', active_org: 'org-1' });
  ...
  it('drops a forged or stale active-org cookie', async () => {
    membershipContext.isActiveMember.mockResolvedValue(false);

    const context = await run({ access_token: 'tok', active_org: 'org-forged' });

    expect(context?.accountId).toBe('user-1');
    expect(context?.activeOrganizationId).toBeUndefined();
  });
