...
import { GetOnboardingStatusUseCase, OnboardingStatus } from '../application/get-onboarding-status.use-case';
...
export class OnboardingController {
  ...
  @Get('status')
  @ApiOperation({ summary: 'Get the onboarding and active-gym status' })
  @ApiOkResponse({ type: OnboardingStatusModel })
  @ApiUnauthorizedResponse({ type: ErrorResponseModel })
  async status(@CurrentUser() user: UserPublicProfile): Promise<OnboardingStatus> {
    return this.getOnboardingStatus.execute(user.id);
  }
}
