...
export class ListRoutinesUseCase {
  ...
  async execute(callerUserId: string, gymId: string): Promise<RoutineView[]> {
    await this.permissions.requirePermission(callerUserId, gymId, PERMISSIONS.ROUTINES_READ);

    let routines = await this.routines.listByGym(gymId);
    ...
    if (caller) {
      ...
      if (callerRole?.key === STUDENT_ROLE_KEY) {
        routines = routines.filter(
          (routine) => routine.scope === RoutineScope.TEMPLATE || routine.ownerMemberId === caller.id,
        );
      }
    }
