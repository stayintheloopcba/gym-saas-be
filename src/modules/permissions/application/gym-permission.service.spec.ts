...
describe('GymPermissionService', () => {
  ...
  it('denies users without an active membership', async () => {
    ...
    await expect(service.requirePermission('user-1', 'gym-1', PERMISSIONS.ORGANIZATION_READ)).rejects.toBeInstanceOf(
  ...
  it('supports OR logic for multiple permissions', async () => {
    ...
    await expect(
      service.checkPermission('user-1', 'gym-1', [PERMISSIONS.ORGANIZATION_UPDATE, PERMISSIONS.ORGANIZATION_READ]),
    ).resolves.toBe(true);
  });
