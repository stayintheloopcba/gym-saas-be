...
import { ACTIVE_ORG_COOKIE } from '../../../common/context/membership-context.port';

/**
 * Helper de la cookie httpOnly que transporta la organización activa.
 *
 * A diferencia de la cookie de refresh (scoped a `/auth/refresh`), `active_org`
 ...
 */
@Injectable()
export class ActiveOrgCookie {
  private readonly cookieName: string;
  private readonly secure: boolean;
  private readonly domain: string;

  constructor(config: ConfigService) {
    this.cookieName = config.get<string>('ACTIVE_ORG_COOKIE', ACTIVE_ORG_COOKIE);
    this.secure = config.get<string>('COOKIE_SECURE') === 'true';
    this.domain = config.get<string>('COOKIE_DOMAIN', 'localhost');
  }

  set(res: Response, organizationId: string): void {
    res.cookie(this.cookieName, organizationId, this.options());
  }
