...
/**
 * Agregado `Organization` (el tenant del producto multi-tenant).
 *
 * Igual que `User`, la entidad de dominio y el modelo de persistencia TypeORM
 * son la misma clase; el acceso pasa siempre por el port `OrganizationRepository`.
 *
 ...
 */
@Index('uq_organizations_slug_active', ['slug'], { unique: true, where: '"deleted_at" IS NULL' })
...
export class Organization extends BaseEntity {
  ...
  /** Tipografía de marca (whitelist `ORGANIZATION_FONTS`). */
